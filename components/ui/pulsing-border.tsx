...
vec3 shade(vec2 uv, vec2 p, float t) {
  // Compute box to match the actual canvas edges at any aspect ratio.
  // p is in aspect-corrected space scaled by u_scale, so the screen corner
  // lands at (resolution / (2 * minRes)) * u_scale. A tiny inset keeps the
  // glow fully inside the canvas.
  float minRes = min(u_resolution.x, u_resolution.y);
  vec2 box = u_scale * vec2(u_resolution.x, u_resolution.y) / (2.0 * minRes) - vec2(0.012);
  vec2 d = abs(p) - box;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const t = window.setTimeout(() => {
        if (pendingContextReleases.get(cvs) !== t) return
        pendingContextReleases.delete(cvs)
        gl.getExtension("WEBGL_lose_context")?.loseContext()
        cvs.width = 1; cvs.height = 1
      }, 0)
      pendingContextReleases.set(cvs, t)
    }
  }, [])
